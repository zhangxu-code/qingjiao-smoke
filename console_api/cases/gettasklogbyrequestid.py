...
class GetTaskLogByRequestID(unittest.TestCase):
    code = """
import privpy as pp
data = pp.ss("data")
...
"""
    ...
    def test_getlog_running(self):
        ...
        response = self.client.get_role_log_byrequestid(query="role=tm&requestId=%s" % requestid)
        logger.info(response)
    ...
    def test_getlog_finished(self):
        ...
        for role in response.get("data"):
            response = self.client.get_role_log_byrequestid(query="role=%s&requestId=%s" % (role, requestid))
            logger.info(response)
            if isinstance(self.check_schema(resp=response), str):
                self.assertTrue(False, "jsonschema check failed")
            self.assertEqual(response.get("code"), 0, msg="expect code = 0")
    ...
    def test_getlog_taskid_notexist(self):
        ...
        self.assertEqual(response.get("code"), 0, msg="expect code = 0")
        for role in response.get("data"):
            response = self.client.get_role_log_byrequestid(query="role=%s&requestId=%shahaha" % (role, requestid))
            logger.info(response)
    ...
    def test_getlog_role_noexist(self):
        ...
        requestid = response.get("data")
        self.assertEqual(response.get("code"), 0, msg="expect code = 0")
        response = self.client.get_role_log_byrequestid(query="role=hahi&requestId=%s" % requestid)
        logger.info(response)
        self.assertEqual(response.get("code"), 1, msg="expect code = 1")
